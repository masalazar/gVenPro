...
public class Funciones {
    ...
    public void EliminarNumerosOnlineFecha(String Fecha, Context context){
        UsuariosSQLiteHelper usdbh =
                new UsuariosSQLiteHelper(context, "ISTEM", null, 1);

        SQLiteDatabase db = usdbh.getWritableDatabase();

        db.execSQL("DELETE FROM NumerosOnline WHERE Fecha < " + Fecha);

        db.close();
    }
    public boolean AgregarNumeroOnline(String IdCategoria,String Fecha, Context context){

        UsuariosSQLiteHelper usdbh = new UsuariosSQLiteHelper(context, "ISTEM", null, 1);

        SQLiteDatabase db = usdbh.getWritableDatabase();

        Cursor Cursor = BuscarNumeroOnline(context,IdCategoria);

        if (Cursor.getCount() > 0){
            db.execSQL("UPDATE NumerosOnlina " +
        ...
        db.execSQL("INSERT INTO NumerosOnline (IdCategoria, Cantidad, Fecha) " +
                "VALUES (" + IdCategoria + ", " + "1" +"," + Fecha);

        db.close();

        return true;
    }
    public Cursor BuscarNumeroOnline(Context context,String IdCategoria){
        UsuariosSQLiteHelper usdbh =
                new UsuariosSQLiteHelper(context, "ISTEM", null, 1);

        SQLiteDatabase db = usdbh.getWritableDatabase();

        String varSQL = "SELECT * From NumerosOnline Where IdCategoria = " + IdCategoria;
    ...
    public void BuscarPublicidad(ejeWS oEjeWS, String vIdSucursal, Context Contexto){
